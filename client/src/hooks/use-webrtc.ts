...
export function useWebRTC() {
  ...
  useEffect(() => {
    ...
    async function startLocalMedia() {
      ...
      try {
        ...
        // Attach new local tracks to all existing peer connections so that
        // others start receiving our media even if the peer existed before
        // we became a speaker.
  ...
  useEffect(() => {
    ...
    const createPeer = (targetId: string, initiator: boolean) => {
      ...
      // Automatically renegotiate whenever tracks or parameters change.
      ...
      // Initial offer from one side to bootstrap the connection.
      if (initiator && socket.connected) {
        pc.createOffer()
          .then(offer => pc.setLocalDescription(offer))
          .then(() => {
            socket.emit('webrtcOffer', { targetId, sdp: pc.localDescription });
    ...
    // Ensure every participant connects to every other participant so that
    // audience members also receive media from speakers.
    // For each other user, whichever of us has the greater ID initiates
    // the connection to avoid crossing offers.
    const targets = users.filter(u => u.id !== currentUser.id);
    targets.forEach(target => {
      if (!peersRef.current[target.id] && currentUser.id > target.id) {
        createPeer(target.id, true);
      }
    });

    // Cleanup disconnected peers (users no longer present)
