...
export function useWebRTC() {
  ...
  useEffect(() => {
    ...
    // If I am a speaker, and another speaker joins, the one with the 'greater' ID initiates
    // to avoid crossing offers.
    if (currentUser.isSpeaker) {
      const otherSpeakers = users.filter(u => u.isSpeaker && u.id !== currentUser.id);
