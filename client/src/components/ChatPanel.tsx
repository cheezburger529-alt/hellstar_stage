...
import { Send, MessageSquareOff, ArrowBigUp, Pin, SmilePlus } from 'lucide-react';
                    ...
                    <div className="pt-1">
                      <Popover>
                        <PopoverTrigger asChild>
                          <button
                            type="button"
                            className="inline-flex items-center gap-1 rounded-full bg-black/20 hover:bg-black/40 text-[11px] px-2 py-0.5 text-white/80"
                          >
                            <SmilePlus className="w-3 h-3" />
                            <span>React</span>
                          </button>
                        </PopoverTrigger>
                        <PopoverContent side="top" className="w-auto px-2 py-1 text-base">
                          <div className="flex flex-wrap gap-1">
                            {COMMON_EMOJIS.map((emoji) => (
                              <button
                                key={emoji}
                                type="button"
                                className="px-1 py-0.5 hover:bg-white/10 rounded-md"
                                onClick={() =>
                                  socket.emit('reactMessage', {
                                    messageId: msg.id,
                                    emoji,
          ...
          <form onSubmit={sendMessage} className="relative flex items-center gap-2">
            ...
            <div className="relative flex-1">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-4 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-white placeholder:text-muted-foreground"
              />
              <button 
                type="submit"
                disabled={!input.trim()}
