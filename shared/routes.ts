...
export const ws = {
  send: {
    ...
    chatMessage: z.object({
      text: z.string(),
      kind: z.enum(['chat', 'question']).optional(),
    }),
    ...
    voteQuestion: z.object({ messageId: z.string() }),
    pinQuestion: z.object({ messageId: z.string() }),
    votePoll: z.object({ optionId: z.string() }),
    reactMessage: z.object({ messageId: z.string(), emoji: z.string() }),
  },
  receive: {
    stateUpdate: z.object({ stage: stageSchema.nullable(), users: z.array(userSchema) }),
    chatMessage: messageSchema,
    questionUpdated: messageSchema,
    messageUpdated: messageSchema,
