...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  // Single active poll timer for this in-memory stage
  ...
  io.on("connection", (socket) => {
      ...
      try {
        ...
        if (isAdmin && payload.text.startsWith("/")) {
          ...
          switch (command) {
            ...
            case "start_vote": {
              if (stage) {
                // Optional duration as first argument, e.g. 30s, 5m, 2h, 1d
                let durationMs: number | null = null;
                let optionsStartIndex = 0;

                const durationMatch = rest[0]?.match(/^(\d+)([smhd])$/i);
                if (durationMatch) {
                ...
                const restText = rest.slice(optionsStartIndex).join(" ");
                ...
                if (rawOptions.length < 2) {
                  await pushSystemMessage(
                    "Usage: /start_vote [duration] Option1, Option2, Option3 (need at least 2 options). Example: /start_vote 2m Yes, No"
                  );
                  break;
                }
                ...
                await pushSystemMessage(
                  `New poll started: ${rawOptions.join(" / ")}${
                    endsAt ? " (auto-ends soon)" : ""
                  }`
                );

                if (durationMs != null) {
                  activePollTimeout = setTimeout(async () => {
                    const latestStage = await storage.getStage();
                    if (!latestStage || !latestStage.activePoll) return;
                    if (latestStage.activePoll.id !== pollId) return;

                    const finishedPoll = latestStage.activePoll;
                    const totalVotes = finishedPoll.options.reduce(
                      (sum, o) => sum + (o.votes || 0),
                      0
                    );
                    ...
                    if (totalVotes === 0) {
                      await pushSystemMessage("Poll ended with no votes.");
                    } else {
                      const maxVotes = Math.max(
                        ...finishedPoll.options.map((o) => o.votes || 0)
                      );
                      const winners = finishedPoll.options.filter(
                        (o) => (o.votes || 0) === maxVotes
                      );
                      const winnerLabels = winners
                        .map((w) => w.label)
                        .join(" / ");
                      const percent = Math.round(
                        (maxVotes / totalVotes) * 100
                      );
                      await pushSystemMessage(
                        `Poll ended. Winner: ${winnerLabels} (${percent}% / ${maxVotes} votes, total ${totalVotes}).`
                      );
                    }
            ...
            case "end_vote":
              if (stage && stage.activePoll) {
                const finishedPoll = stage.activePoll;
                const totalVotes = finishedPoll.options.reduce(
                  (sum, o) => sum + (o.votes || 0),
                  0
                );
            ...
            case "help":
              await pushSystemMessage(
                "Chat commands: /lock, /unlock, /qna on, /qna off, /start_vote [duration] <opt1, opt2,...>, /end_vote, /help"
              );
              break;
          }
    ...
    socket.on("votePoll", async (data) => {
      // Require a concrete userId so we can enforce one-vote-per-user
      if (!userId) return;
      try {
        ...
        // Already voted in this poll? Ignore.
        ...
        await storage.updateStage({
          activePoll: { ...poll, options, voters: [...poll.voters, userId] },
        });
        await broadcastStateUpdate();
      } catch (err) {
    ...
    socket.on("reactMessage", async (data) => {
      if (!userId && !isAdmin) return;
      try {
        const payload = ws.send.reactMessage.parse(data);
        const msg = await storage.getMessage(payload.messageId);
        if (!msg) return;
        ...
        if (existingIndex >= 0) {
          ...
          if (hasReacted && userId) {
            // Toggle off: remove user from this emoji
            const updatedUsers = existing.userIds.filter((id) => id !== userId);
            ...
            // Add this user to existing reaction
            newReactions[existingIndex] = {
              ...existing,
          ...
          // First time this emoji appears on this message
          newReactions.push({
            emoji: payload.emoji,
            userIds: [userId],
          });
